...
do
    ...
    echo $word > $file
    java --add-exports java.base/jdk.internal.misc=ALL-UNNAMED $mem -cp $cp $clz  -iterations=$iter -warmup-time=$warmupTime -testRunMillis=$testTime -include=$word data/media.1.json >> $file || code=$?
    if [[ $code -ne 0 ]]; then
